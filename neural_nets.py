...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        Vectorized_output_activation_derivative=np.vectorize(output_layer_activation_derivative)
        hidden_layer_error = (self.hidden_to_output_weights.T@ Vectorized_output_activation_derivative(output) )*output_layer_error# (3 by 1 matrix)

        RELU_derivative=np.vectorize(rectified_linear_unit_derivative)
        bias_gradients =np.multiply(RELU_derivative(hidden_layer_activation),hidden_layer_error) # 
...
#pragma: coderesponse end
    def predict(self, x1, x2):
        ...
        vectorized_layer_activation=np.vectorize(output_layer_activation)
        activated_output = vectorized_layer_activation(output) # 
